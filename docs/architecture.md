...
**그리고 값에 따라 다르다** (`processing/follower-cache.ts` 의 `ttlFor`):

| 캐시된 값 | TTL | 왜 |
|---|---|---|
| `true` | **30일** | 팔로우는 잘 안 끊긴다. 그 사이 언팔했어도 손해는 양식 한 번 더 |
| `false` · 모름 | **1시간** | **우리 1차 DM 이 "팔로우해달라"고 시키는 값이다.** 시키는 대로 한 사람에게 30일 동안 같은 걸 또 묻게 된다 |

만료됐고 대화가 있으면 **댓글 시점에 다시 조회한다** (`context.ts` 의 `refreshFollowerCache`).
`is_user_follow_business` 는 대화 성립 후에만 되므로 처음 보는 사람에게는 이 길이 없고,
그 경우는 그대로 2단계로 간다.
